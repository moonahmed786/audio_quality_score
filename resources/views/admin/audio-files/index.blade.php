<x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <!-- Search Bar -->
            <div class="bg-white/80 backdrop-blur-md rounded-3xl border border-gray-100 p-4 mb-6 shadow-sm">
                <form method="GET" action="{{ route('admin.audio-files.index') }}" class="flex gap-3">
                    ...
                    <button type="submit"
                            class="inline-flex items-center px-5 py-2.5 bg-violet-600 border border-transparent rounded-xl font-bold text-sm text-white hover:bg-violet-700 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:ring-offset-2 shadow-sm transition">
                        Search
                    </button>
                    @if (request('search'))
            ...
            <!-- Table -->
            <div class="bg-white/80 backdrop-blur-md rounded-3xl border border-gray-100 overflow-hidden shadow-sm">
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-100">
                        <thead class="bg-gray-50/70">
                            <tr>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider w-24">Player</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-50">
                            @forelse ($files as $file)
                                <tr class="hover:bg-violet-50/50 transition-colors group">
                                    <td class="px-6 py-4" title="{{ $file->filename }}">
                                        <x-audio-player :src="asset('storage/' . $file->file_path)" :id="$file->id" minimal="true" />
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-semibold text-gray-900">{{ \Illuminate\Support\Str::limit($file->title, 20) }}</div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-right text-sm font-medium">
                                        <div class="flex flex-col items-end gap-2">
                                            <div class="text-xs text-gray-500 font-medium">
                                                ...
                                                @endif
                                                &middot; {{ $file->created_at->format('M d, Y') }}
                                            </div>
                                            <div class="inline-flex items-center justify-end gap-2">
                                            <a href="{{ route('admin.audio-files.edit', $file) }}"
                                               class="inline-flex items-center gap-1 px-3 py-1.5 bg-violet-50 text-violet-700 rounded-lg text-xs font-bold hover:bg-violet-100 hover:text-violet-800 transition shadow-sm shrink-0">
                                            ...
                                            <form method="POST" action="{{ route('admin.audio-files.destroy', $file) }}" class="inline shrink-0"
                                                ...
                                                @method('DELETE')
                                                <button type="submit"
                                                        class="inline-flex items-center gap-1 px-3 py-1.5 bg-rose-50 text-rose-700 rounded-lg text-xs font-bold hover:bg-rose-100 hover:text-rose-800 transition shadow-sm">
                                                    <svg class="w-3.5 h-3.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
