                ...
                <form method="GET" action="{{ route('admin.audio-files.index') }}" class="flex gap-3">
                    <div class="flex-1 relative">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}"
                               ...
                               class="w-full pl-10 pr-4 py-2.5 rounded-xl border-gray-200 focus:border-violet-500 focus:ring-violet-500 text-sm transition">
                    </div>
                    <button type="submit"
                            class="inline-flex items-center px-5 py-2.5 bg-slate-900 border border-transparent rounded-xl font-medium text-sm text-white hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-slate-500 focus:ring-offset-2 transition shadow-sm">
                        Search
                    </button>
                    @if (request('search'))
                        <a href="{{ route('admin.audio-files.index') }}"
                           class="inline-flex items-center px-5 py-2.5 bg-white border border-gray-200 rounded-xl font-medium text-sm text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:ring-offset-2 transition">
                            Clear
                        </a>
                    @endif
                </form>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition">
                                            <a href="{{ route('admin.audio-files.edit', $file) }}"
                                               class="inline-flex items-center gap-1 px-3 py-1.5 bg-violet-50 text-violet-700 rounded-lg text-xs font-semibold hover:bg-violet-100 transition">
                                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                                </svg>
                                                Edit
                                            </a>
                                            <form method="POST" action="{{ route('admin.audio-files.destroy', $file) }}" class="inline"
                                                  onsubmit="return confirm('Delete this audio file permanently?');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                                        class="inline-flex items-center gap-1 px-3 py-1.5 bg-rose-50 text-rose-700 rounded-lg text-xs font-semibold hover:bg-rose-100 transition">
                                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
